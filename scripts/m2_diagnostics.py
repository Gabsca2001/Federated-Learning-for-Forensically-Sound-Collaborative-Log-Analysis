...
the train and validation splits. Test and temporal-holdout metrics are computed
once, after the fixed number of epochs has completed.
"""
...
def _plot_learning_curves(
    *, plt: Any, history: list[dict[str, Any]], output: Path
) -> None:
...
def run_diagnostics(
    *,
    ...
    best_loss_row = min(history, key=lambda item: item["validation_weighted_log_loss"])
    ...
    summary = {
        ...
        "selection_policy": (
            "monitor-only; fixed epochs; test evaluated once after training"
            if include_test
            else "monitor-only; fixed epochs; test and temporal holdout remain sealed"
        ),
        "limitations": [
    ...
    _write_json(output / "training_history.json", {"epochs": history})
    _write_json(output / "final_metrics.json", final_metrics)
    _write_json(output / "summary.json", summary)
    _plot_learning_curves(
        plt=plt, history=history, output=output / "learning_curves.png"
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--include-test",
        action="store_true",
        help="Evaluate test and temporal holdout once after fixed-epoch training.",
    )
