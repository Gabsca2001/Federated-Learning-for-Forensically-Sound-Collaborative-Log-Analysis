...
| Byzantine attacks and robust aggregation | Implemented; runtime campaign in progress (M6) | Verified `f=3` amplification, model replacement, label flip, sign flip, Gaussian noise, and backdoor scenarios over identical frozen-input profiles; backdoor client ASR 0.99514–0.99828 versus zero for benign clients and every aggregate; collusion, prototype artifacts, other `f` values, and repeated seeds pending |
